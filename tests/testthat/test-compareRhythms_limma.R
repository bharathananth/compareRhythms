...
test_that("limma analysis works for different input params", {
  expect_warning(compareRhythms(expr, exp_design, period = 12, method = "limma"))
  ...
  expect_warning(compareRhythms(expr, exp_design, rhythm_fdr = 0, method = "limma"))
...
test_that("limma analysis works for different input params with batch", {
  expect_warning(compareRhythms(expr, exp_design_batch, period = 12, method = "limma"))
  ...
  expect_warning(compareRhythms(expr, exp_design_batch, rhythm_fdr = 0, method = "limma"))
...
test_that("limma analysis runs for arrhythmic dataset", {
  dim_y <- dim(expr)
  y_null <- matrix(rnorm(prod(dim_y)), nrow = dim_y[1], ncol = dim_y[2])
  colnames(y_null) <- colnames(expr)
  rownames(y_null) <- rownames(expr)
  expect_equal(nrow(expect_warning(compareRhythms(y_null, exp_design, method = "limma"))), 0)
})
