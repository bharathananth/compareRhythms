...
test_that("DESeq2 analysis works for different input params", {
  ...
  expect_warning(compareRhythms(countsFromAbundance, exp_design, rhythm_fdr = 0, method = "deseq2"))
...
test_that("DESeq2 analysis works for non-rhythmic data", {
  expect_equal(nrow(expect_warning(compareRhythms(counts_norhythm, exp_design_norhythm, method="deseq2"))), 0)
  expect_s3_class(expect_warning(compareRhythms(counts_norhythm, exp_design_norhythm, method="deseq2")), "data.frame")
})
