...
test_that("DODR analysis works for different input params", {
  expect_warning(compareRhythms(expr, exp_design, period = 12, method = "dodr"))
  ...
  expect_warning(compareRhythms(expr, exp_design, rhythm_fdr = 0, method = "dodr"))
  ...
  expect_named(results,
               c("id", "category", "rhythmic_in_CC", "rhythmic_in_KD", "diff_rhythmic", "CC_amp",
                 "CC_phase", "KD_amp", "KD_phase", "adj_p_val_CC", "adj_p_val_KD", "adj_p_val_DR"))
})
...
test_that("DODR analysis runs for arrhythmic dataset", {
  ...
  expect_warning(compareRhythms(y_null, exp_design, method = "dodr"))
})
